...
<style>
  .mermaid {
    ...
    width: 100%;
    margin-bottom: 2rem; /* Adds space at the bottom to prevent clipping */
  }
  .mermaid svg {
...
<div class="mermaid" style="overflow: visible; height: auto; min-height: 800px;">
  flowchart TD
    ...
    subgraph Store1 ["Store 1: Christopher (Local Machine)"]
        ...
        ChrisCron -.->|Trigger| ChrisExtract
        ChrisDB -->|Raw Data| ChrisExtract
        ChrisExtract -->|Preprocessed Data| ChrisCSV
        ChrisCSV -->|Read| ChrisUpload
    ...
    subgraph Store2 ["Store 2: Rebecca (Local Machine)"]
        ...
        RebCron -.->|Trigger| RebExtract
        RebDB -->|Raw Data| RebExtract
        RebExtract -->|Preprocessed Data| RebCSV
        RebCSV -->|Read| RebUpload
    ...
    subgraph Dashboard ["Dashboard Application"]
        ...
        DownloadScript --> DataFolder
        DataFolder --> Ops
        Ops --> UI
    end

    %% Convergence and Final Flow
    ChrisUpload -->|Secure Upload| GDrive
    RebUpload -->|Secure Upload| GDrive
    GDrive -->|Fetch Latest| DownloadScript
</div>
{% endraw %}
...
* **A/B Testing Experimentation:** With granular data, I can now run controlled experiments. For example, as previously mentioned, a target discount outreach experiment may look like this:
    * *Hypothesis:* Targeted discounts will reactivate at risk high value customers.
    * *Control Group:* 50% of at risk customers receive no text.
    * *Treatment Group:* 50% of at risk customers receive a text.
    * *Metric:* Return rate within 7 days.
